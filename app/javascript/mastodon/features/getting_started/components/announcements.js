  ...
  renderAnnouncement (announcement) {
    ...
    return (
      <div key={announcement.get('id')} className='announcements__item'>
        ...
        <IconButton title={intl.formatMessage(messages.close)} icon='times' className='announcements__item__dismiss-icon' />
      </div>
    );
  }
    ...
    return (
      <div className='announcements'>
        ...
        <div className='announcements__pagination'>
          ...
          <IconButton title={intl.formatMessage(messages.previous)} icon='chevron-left' onClick={this.handlePrevClick} size={13} />
          <span>{index + 1} / {announcements.size}</span>
          <IconButton title={intl.formatMessage(messages.next)} icon='chevron-right' onClick={this.handleNextClick} size={13} />
        </div>
      </div>
    );
  }
